"""Tests for the AiiDA exploration agent wiring (no real LLM).
...
Provider selection in ``get_model`` is unit-tested separately in
``tests/agents/test_models.py`` and is not duplicated here. What can't be
unit-tested, and isn't here: whether the model picks the *right* tool for a
phrasing, or answer quality. That is a real-model evaluation, not a mock.
"""
