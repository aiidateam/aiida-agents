...
``check`` and ``warm`` used to live here too. They are now ``doctor --only
model`` and ``doctor --only model --warm``: both were doing what one of
``doctor``'s rows already did, and three diagnostic commands meant working out
which of them to reach for (issue #75).
"""
