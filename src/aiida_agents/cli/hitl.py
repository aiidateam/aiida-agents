...
tool therefore needs a main-thread executor, not just ``submit_workflow``:
``_run_approvals`` calls the tool's own registered function, and submissions get
an extra resolve-and-preview pass on top (see ``_triage_submissions``).
"""
