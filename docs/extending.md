...
Read [Architecture](/docs/architecture.md) first if you have not, since the constraints below follow from it.
...
**Never invent a value the caller could mistake for a queried one.** A default that gets echoed back, such as a `structure_type` nobody asked for or a "recommended" item nothing ranked, is indistinguishable from real data once it reaches the model. If you cannot support a value, do not return a field for it.
...
**Before adding one, check the alternative.** A new agent is justified when a domain needs its own tool surface *and* its own prompt. If it needs neither, and you are really just adding capability, a tool on an existing agent is cheaper, has no routing cost, and cannot be mis-routed to.
